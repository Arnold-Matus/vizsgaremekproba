...
class seeder1 extends Seeder
{
    ...
    /**CREATE TRIGGER `zenebekeresfeltoltesorarendbe2` AFTER INSERT ON `keres`
 FOR EACH ROW begin if ( new.validalte && (select lejatszhatoe from zene where zene.keresurl = new.zeneurl limit 1 )) then 
 set @zeneidd=(select id from zene where zene.keresurl=new.zeneurl limit 1); 
 set @mikortoll= (select orarend.meddig from orarend order by orarend.mikortol desc limit 1); 
 set @hosss=(SELECT zene.hossz from zene WHERE zene.keresurl=NEW.zeneurl LIMIT 1);
 
 set @meddig= ( TIMESTAMPADD(SECOND,(@hossz), (@mikortoll)));
 set @szunett=(SELECT COUNT(szunetek.hanyadik) FROM szunetek WHERE szunetek.kezdes<@mikortoll and szunetek.vege>@meddig);
 if (@szunett=1) THEN
 INSERT INTO orarend(orarend.zeneid,orarend.mikortol,orarend.meddig) VALUES (@zeneidd,@meddig,@szunett);
 end if;
END if; 
